...
class PageLoader:
    ...
    Example:
        from ocr2md.config import PageLoaderConfig
    ...
    def iter_pages_with_unit_indices(
        ...
        if isinstance(sources, str):
            sources = [sources]
        for unit_idx, source in enumerate(sources):
            current_page_indices = page_indices if unit_idx == 0 else None
            for page in self._iter_source(source, page_indices=current_page_indices):
                yield page, unit_idx

    def _iter_source(self, source: str, page_indices: Optional[List[int]] = None):
        ...
        if os.path.isfile(file_path) and file_path.lower().endswith(".pdf"):
            yield from self._iter_pdf(file_path, page_indices=page_indices)
        else:
    ...
    def _iter_pdf(self, file_path: str, page_indices: Optional[List[int]] = None):
