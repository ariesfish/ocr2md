...
from ..utils.markdown_utils import (
    crop_and_replace_images,
    export_crops_from_json_result,
    extract_image_refs,
    replace_markdown_image_refs_with_json_exports,
)
...
class BaseParserResult(ABC):
    ...
    def _save_json_and_markdown(self, output_dir: Union[str, Path]) -> None:
        ...
        # Export region crops directly from JSON bbox metadata.
        if self.original_images:
            try:
                ...
                logger.warning("Failed to export JSON region crops: %s", e)

        # Markdown (reuse JSON-exported crops first, fallback to crop only unmatched refs)
        if self.markdown_result and self.markdown_result.strip():
            md_text = self.markdown_result
            if self.original_images:
                try:
                    imgs_dir = output_path / "imgs"
                    md_text = replace_markdown_image_refs_with_json_exports(
                        md_text,
                        self.json_result,
                        image_prefix="region",
                    )
                    if extract_image_refs(md_text):
                    ...
                    logger.warning("Failed to process markdown image regions: %s", e)
