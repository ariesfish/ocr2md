...
def apply_layout_postprocess(
    ...
    Returns:
        List of lists, each containing normalized layout detection dicts:
        {
            'cls_id': int,
            'label': str,
            'score': float,
    ...
    for img_idx, result in enumerate(raw_results):
        ...
        # Convert to the intermediate layout result format used by the pipeline
        ...
        layout_format_results.append(image_results)

    return layout_format_results
