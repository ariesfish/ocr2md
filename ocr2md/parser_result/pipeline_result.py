...
class PipelineResult(BaseParserResult):
    ...
    def _collect_layout_visualization_files(self) -> List[str]:
        """Collect layout overlay image file paths if available."""
        if not self.layout_vis_dir:
            return []

        layout_root = Path(self.layout_vis_dir)
        ...
        if self.layout_image_indices is not None:
            for idx in self.layout_image_indices:
                for ext in (".jpg", ".png"):
                    candidate = layout_root / f"layout_page{idx}{ext}"
                    if candidate.exists():
                        files.append(candidate)
                        break
        else:
            files.extend(sorted(layout_root.glob("layout_page*.jpg")))
            files.extend(sorted(layout_root.glob("layout_page*.png")))

        return [str(path.resolve()) for path in files]

    def to_dict(self) -> dict:
        """Return structured result payload with layout visualization metadata."""
        payload = super().to_dict()
        payload["layout_visualization"] = {
            "dir": self.layout_vis_dir,
            "files": self._collect_layout_visualization_files(),
        }
        return payload
