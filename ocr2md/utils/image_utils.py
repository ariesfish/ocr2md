...
def pdf_to_images_pil_iter(
    ...
    Args:
        ...
        start_page_id: Start page index (0-based).
        end_page_id: End page index (inclusive); None = last page.
        page_ids: Explicit page indices to render in order.
    ...
    try:
        ...
        if page_ids is None:
            ...
            if end_page_id >= page_count:
                end_page_id = page_count - 1
            page_ids = list(range(start_page_id, end_page_id + 1))
        else:
            page_ids = [page_id for page_id in page_ids if 0 <= page_id < page_count]
        for i in page_ids:
            page = pdf[i]
            try:
                image, _ = _page_to_image(
